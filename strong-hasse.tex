...
We can show this explicitly for any $p>2$ by considering the polynomial $f(x)=x^2-(1-p)$, hence $f'(x)=2x$, for some prime $p$, then $f(1)=1-1+p\equiv0(\mod{p})$, and $f'(1)=2\not\equiv0(\mod{p})$, thus by Hensel’s lemma, there exists $a$ such that $f(a)=0$, which means that $a^2=1-p$, but then $b=a^2+1^2(p-2)$ hence $b+1=0\implies{b}=-1$.
