...
Denote $K$ the scalar field, where either $K=\mathbb{Q}$ or $K=\mathbb{Q}_p$ (including $p=\infty$).
\section*{$n=2$}
Consider the quadratic form $q=a_1x_1^2+a_2x_2^2$, and assume that $a_1=1$ (this will be the assumption for this case and for the next one)\footnote{otherwise divide $q$ by $a_1$, then if $v\in{K}$ solution of $q$ is also a solution of $\frac{1}{a_1}q$}.\\
