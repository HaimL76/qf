...
We claim (without proof) that the squared $p$-adic rationals form an open set\footnote{Denote this set by $S$, then \[S=\{x\in\mathbb{Q}_p : \exists{y}\in\mathbb{Q}_p,\,x=y^2\}=\bigcup_{k=\mathbb{Z}}\{x\in\mathbb{Q}_p : v_p(x)=2k\}=\bigcup_{k=\mathbb{Z}}\{x\in\mathbb{Q}_p : |x|_p=p^{-2k}\}\]
