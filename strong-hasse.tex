...
Consider an $n$-dimensional quadratic form $(V,q)$, where $n=m+1=2k+1$ for some $k\in\mathbb{Z}$, if there exists some $v\in{V}$ such that $q(v)=a$ for some scalar $a\in{K}$, then let $U=Kv<V$ be a one-dimensional subspace of $V$, and we construct a subspace $H<V$ by taking the orthogonal complement to $U$, and we consider the $m$-dimensional quadratic space $(H,h)$, where $h=\langle1,-1,1,-1,\dots,1,-1\rangle$, for which the vector $v=(1,1,\dots,1)\in{K}^m$ is a nontrivial solution because $h(v)=1^2-1^2+\cdots+1^2-1^2=0$, hence the quadratic form $q'=h\perp\langle{a}\rangle=\langle1,-1,\dots,1,-1,a\rangle=h+ax_{m+1}^2$ has $0$ and $a$ as values\footnote{take the vector $u=(1,1,\dots,1)\in{K}^{m+1}$, then $q'(u)=1^2-1^2+1^2-1^2+\cdots+1^2a=a$, or take the vector $w=(1,1,\dots,1,0)$, then $q'(w)=1^2-1^2+1^2-1^2+\cdots+0a=0$}, but then, the quadratic form $q''=q'\perp\langle-a\rangle=q'-ax_{m+2}^2$ has , for the vector $v''=(1,1,\dots,1)\in{K}^{m+2}$, the value $q''(v'')=1^2-1^2+\cdots+a-a=0$.\\
Now suppose that there exists an arbitrary vector $u=(u_1,u_2,\dots,u_{m+2})\in{K}^{m+2}$ such that $q''(u)=0$ and denote $u'=(u_1,u_2,\dots,u_{m+1})\in{K}^{m+1}$, then either $u=(u_1,u_2,\dots,u_n,0)$ and $q''(u)=q'(u')-ax_{m+2}^2$


such that $q(u_1,u_2,\dots,u_n)=0$, or $u=(u_1,u_2,\dots,u_n,1)$ such that $q(u_1,u_2,\dots,u_n)=a$, thus we get a circular reasoning:\\ $a$ is a value of $q$ $\implies$ $q\sim{h}\perp\langle{a}\rangle$ $\implies$ zero is a value of $q-ax_{n}^2$ $\implies$ $a$ is a value of $q$.\\
