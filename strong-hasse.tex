...
Let 
\begin{align*}
q'&=a_1x_1^2+a_2x_2^2+\cdots+a_kx_k^2\\ q''&=a_{k+1}x_{k+1}^2+a_{k+2}x_{k+2}^2+\cdots+a_nx_n^2
\end{align*}
be two quadratic forms such that $q=q'-q''$ has a nontrivial solution $v\in{K}^n$, then there exist $v'\in{K}^k$ and $v''\in{K}^{n-k}$ and $a\in{K}^{\ast}$ such that $q'(v')=q''(v'')=a$.
\end{proposition}
\begin{proof}
...
Now we assume there exist two quadratic forms $q',q''$ of the same dimension, and we denote the quadratic form of their difference by $q=q'-q''$. We saw earlier that $a$ is a value of $q'$ and of $q''$ if and only if $0$ is a value of $q'-ax_{m+1}^2$ and of $q''-ax_{m+1}^2$.
We extend $q$ to a $2(n+1)$-dimensional quadratic form $q=q'\perp-q''$, suppose that there exist vectors $u,w\in{K}^{2(m+1)}$ such that $q'(u)=q''(w)=a\in{K}$, then we denote 
\begin{align*}
