...
\end{proof}
\begin{proposition}
Let $q$ be a quadratic form which has a nontrivial solution in $\mathbb{Q}_p$ for every prime $p$, then there exists $a\in\mathbb{Q}_p$ such that $a_1x_1^2+a_2x_2^2=a$ and $a_3x_3^2+a_4x_4^2=a$.
\end{proposition}
\begin{proof}
...
Consider an $n$-dimensional quadratic form $(V,q)$, where $n=m+1=2k+1$ for some $k\in\mathbb{Z}$, if there exists some $v\in{V}$ such that $q(v)=a$ for some scalar $a\in{K}$, then let $U=Kv<V$ be a one-dimensional subspace of $V$, and we construct an $m$-dimensional subspace $H<V$ by taking the orthogonal complement of $U$ in $V$, and we consider the $m$-dimensional quadratic space $(H,h)$, where $h=\langle1,-1,1,-1,\dots,1,-1\rangle$, for which the vector $v=(1,1,\dots,1)\in{K}^m$ is a nontrivial solution because $h(v)=1^2-1^2+\cdots+1^2-1^2=0$, hence $q\sim{h}\perp\langle{a}\rangle=\langle1,-1,\dots,1,-1,a\rangle=h+ax_{m+1}^2$ has $0$ and $a$ as values\footnote{take the vector $u=(1,1,\dots,1,1)\in{K}^{m+1}$, then $q(u)=1^2-1^2+1^2-1^2+\cdots+1^2a=a$, or take the vector $w=(1,1,\dots,1,0)\in{K}^{m+1}$, then $q(w)=1^2-1^2+1^2-1^2+\cdots+0a=0$}, but then, the quadratic form $q'=q\perp\langle-a\rangle=q-ax_{m+2}^2$ has again $0$ and $a$ as values\footnote{because if  $q(v)=a$ then taking $v_{m+1}=0$ yields $q(v)-a0=a-0=a$, and taking $v_{m+1}=1$ yields $q(v)-a1=a-a=0$, and if $q(v)=0$ then taking $v_{m+1}=0$ yields $q(v)-a0=0-0=0$.}.\\
