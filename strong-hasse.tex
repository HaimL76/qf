...
We claim (without proof) that the squared $p$-adic rationals $(\mathbb{Q}_p^{\ast})^2$ form an open set\footnote{Because $x\in\mathbb{Q}_p$ is a square if and only if $x=p^{2k}y^2$, for some $k\in\mathbb{Z}$ and some  invertible $p$-adic integer $y\in\mathbb{Z}_p^{\ast}$, then $(\mathbb{Q}_p^{\ast})^2$ is a subset of all the $p$-adic numbers whose valuation is an even integer, which we denote by $S$, then \[S=\bigcup_{k=\mathbb{Z}}\{x=p^{2k}y : y\in\mathbb{Z}_p^{\ast}\}=\bigcup_{k=\mathbb{Z}}\{x\in\mathbb{Q}_p : v_p(x)=2k\}=\bigcup_{k=\mathbb{Z}}\{x\in\mathbb{Q}_p : |x|_p=p^{-2k}\}\]
Denote $r=p^{-2k}$. For every $l\in\mathbb{R}$ such that $l<r$, and every $y\in\mathbb{Q}_p$ such that $|x-y|_p<l<r$ we have that $|y|_p=|x-y-x|_p\leq\max\{|x-y|_p,|x|_p\}=\max\{l,r\}=r$, but since $l<r$, then $|x|_p=|x-y+y|_p\leq\max\{|x-y|_p,|y|_p\}$, but all this means that $|y|_p=|x|_p=r$, that is $y\in\{x\in\mathbb{Q}_p : |x|_p=p^{-2k}\}$, which means that every ball around $x$ of radius $l$ is entirely contained in $S$, that is $s$ is an open ball therefore an open set in $\mathbb{Q}_p$, hence an infinite union of sets of this form is also open.} in $\mathbb{Q}_p^{\ast}$, and therefore, due to the approximation theorem, there exists $u=(u_1,u_2)\in\mathbb{Q}^2$ such that if $h(u)=a\in\mathbb{Q}$, then $\frac{a_p}{a}$ is a square in $\mathbb{Q}_p^{\ast}$, but then the quadratic form $g'=ax_{n+1}^2-g$ has a solution\footnote{The quadratic form $g=q-h$ is of dimension $n-2$, and we added $ax_{n+1}^2$.}  $w=(v_3,v_4,\dots,v_n,\sqrt{\frac{a_p}{a}})\in\mathbb{Q}_p^{n-1}$, because $g'(w)=a\frac{a_p}{a}-g(v'')=a_p-a_p=0$, and since $g'$ is of dimension $n-1$, then by the assumption it also has a nontrivial solution $w'=(v'_3,v'_4,\dots,v'_n,v'_{n+1})\in\mathbb{Q}^{n-1}$, but since $g=ax_{n+1}^2-g'$, the vector $w''=(v'_3,v'_4,\dots,v'_n,v'_{n+1},1)$ yields $g(w'')=a-g'(w')=a$, thus the vector $u'=(u_1,u_2,v'_1,v'_2,\dots,v'_n,v'_{n+1},1)$ yields $q(u')=h(u)-g(w'')=a-a=0$, which concludes  the proof for all $n>4$, and so we get this result for every $n>1$.
\end{proof}
