...
\section*{$n=2$}
Consider the quadratic form $q=a_1x_1^2+a_2x_2^2$, and assume that $a_1=1$ (this will be the assumption for this case and for the next one)\footnote{otherwise divide $q$ by $a_1$, then any solution of $q$ is also a solution of $\frac{1}{a_1}q$}.\\
If $v=(v_1,v_2)\in\mathbb{R}^2$ is a solution of $q$, then $q(v)=v_1^2+a_2v_2^2=0\iff{a_2=-(\frac{v_1}{v_2}})^2$, thus $-a_2>0$ (because it is a square in $\mathbb{R}$)\footnote{This is true for $\mathbb{R}$ because it has an ordering while $\mathbb{Q}_p$, for any prime $p$, does not (as follows from the Artin–Schreier theorem).}, so we can reformat $q$ as $q=x_1^2-a_2x_2^2$, where $a_2>0$.\\
\newline
