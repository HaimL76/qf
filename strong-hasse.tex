...
Without loss of generality we assume that $|a_2|\leq|a_3|$. Since $a_2$ and $a_3$ are integers, then their absolute values are positive integers\footnote{while this assertion is true for (the integers of) any field $K$, it is relevant here only for $\mathbb{Q}$, because this is the  target of this proof.}, thus their sum is a positive integer, and we denote $m=|a_2|+|a_3|\geq2$.\\
