...
Without loss of generality we assume that $|a_2|\leq|a_3|$. Since $a_2$ and $a_3$ are integers, then their absolute values (either in $\mathbb{R}$ or $\mathbb{Q}_p$ for all $p$) are positive integers\footnote{For $\mathbb{Q}_{p_i}$, if $a=p_1p_2\cdots{p}_{k-1}p_k$, then \[|a|_{p_i}=\left|\prod_{j=1}^kp_j\right|_{p_i}=\prod_{j=1}^k\left|p_j\right|_{p_i}=p_i^{v_{p_i}(p_i)}\prod_{j\neq{i}}p_j^{v_{p_i}(p_j)}=p_i\prod_{j\neq{i}}1=p_i,\]
because $v_{p_i}(p_j)=\begin{cases}
1,&j=i\\
0,&j\neq{i}\\
\end{cases}$}, thus their sum is a positive integer, and we denote $m=|a_2|+|a_3|\geq2$.\\
