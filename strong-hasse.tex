...
\end{theorem}
\begin{proof}
The only if direction is trivial, since $\mathbb{Q}\subset\mathbb{R}$ and $\mathbb{Q}\subset\mathbb{Q}_p$ for every prime\footnote{the image of every $r\in\mathbb{Q}$ in either of its completion fields is the equivalence class of the constant Cauchy sequence $r,r,\dots,r,\dots$}, but the if direction is complicated and requires separate proofs for several cases of $n$.
\section*{$n=2$}
Consider the quadratic form $q=a_1x_1^2+a_2x_2^2$, and assume that $a_1=1$ (this will be the assumption for this case and for the next one)\footnote{otherwise divide $q$ by $a_1$, then for all $v\in{K}^2$ we have $q(v)=0\iff\frac{1}{a_1}q(v)=0$, and we take $\frac{1}{a_1}q$ to be $q$.}.\\
...
We can show this explicitly, for instance we take any $p>2$, by considering the polynomial $f(x)=x^2-(1-p)$, hence $f'(x)=2x$, for some prime $p$, then $f(1)=1-1+p\equiv0(\mod{p})$, and $f'(1)=2\not\equiv0(\mod{p})$, thus by Hensel’s lemma, there exists $a\in\mathbb{Q}_p$ such that $f(a)=0$, which means that $a^2=1-p$, but then $a^2+\underset{p-2}{\underbrace{1^2+1^2+\cdots+1^2}}=-1$.
...
a_2=\prod_pp^{v_p(a_2)}=\prod_pp^{v_p(r^2)}=\prod_pp^{2v_p(r)}=\prod_p\left(p^{v_p(r)}\right)^2=\left(\prod_pp^{v_p(r)}\right)^2.
\]
But $p^{v_p(r)}\in\mathbb{Q}$, for every prime $p$, hence $a_2$ is a square in $\mathbb{Q}$, that is $a_2=a_2'^2$ for some $a_2'\in\mathbb{Q}$, so for any vector of the form $v'=(v_1,\frac{v_1}{a_2'})\in\mathbb{Q}^2$ we have $q(v')=v_1^2-a_2(\frac{v_1}{a_2'})^2=v_1^2-a_2\frac{v_1^2}{a_2}=v_1^2-v_1^2=0$.
\section*{$n=3$}
...
Without loss of generality we assume that $|a_2|\leq|a_3|$. Since $a_2$ and $a_3$ are integers, their absolute values are positive integers, hence their sum is a positive integer, and we denote $m=|a_2|+|a_3|\geq2$.\\
...
Since $a_3$ is a square free integer, that is $a_3=p_1p_2\cdots{p}_k$ for $k$ distinct primes, then for an arbitrary prime $p_i$ of these primes, denote $p'=p_1p_2\cdots{p}_{i-1}p_{i+1}\cdots{p}_k$, thus $a_3=\pm{p}_ip'$, and there exists $v=(v_1,v_2,v_3)\in\mathbb{Q}_{p_i}^3$ such that $q(v)=v_1^2-a_2v_2^2-a_3v_3^2=0$, because $q$ has nontrivial solutions for all primes $p$, which means that ${v}_1^2-a_2v_2^2=a_3v_3^2=p_ip'v_3^2\equiv0(\mod{p_i})$. We assume that $v$ is primitive, that is, the common divisor of $v_1,v_2,v_3$ is $d=1$ (that is, $v_1,v_2,v_3$ are relatively prime)\footnote{otherwise $d>1$, and we can divide by it to obtain $u=\frac{v}{d}$, but $u_1^2-a_2u_2^2-a_3u_3^2=\frac{1}{d^2}(v_1^2-a_2v_2^2-a_3v_3^2)=0$, hence $q(v)=0\iff{q}(u)=0$, and we can take $u$ to be $v$.}.\\
\newline
...
and $f$ is a bijection as follows from (the existence of a unique solution modulo $a_3$ to a system of congruences modulo $p_1,p_2,\dots,p_k$ as given by) the Chinese Remainder Theorem.
...
There exists a bijection between the set of rational solutions of the (original) equation $q=x_1^2-a_2x^2-a_3x_3^2=0$ and that of  $q'=x_1^2-a_2x_2^2-yx_3^2=0$ (a full proof is given later in this document), but then $m'=|a_2|+|y|<|a_2|+|a_3|=m$, and we assumed that for every $m'<m$ there exists a solution of the equation $q'$ in $\mathbb{Q}_p^3$ for every prime $p$ if and only if there exists a solution of $q'$ in $\mathbb{Q}^3$, thus by the bijection above there exists a solutions of the equation $q$ in $\mathbb{Q}$ if and only if there exists a solution of $q$ in $\mathbb{Q}_p$ for every prime $p$, which proves\footnote{To put it more clearly\\ 
(assumption) $q$ has a solution in all $\mathbb{Q}_p^3$ $\implies$ (bijection) $q'$ has a solution in all $\mathbb{Q}_p^3$\\ $\implies$ (induction base) $q'$ has a solution in $\mathbb{Q}^3$ $\implies$ (bijection inverse) $q$ has a solution in $\mathbb{Q}^3$} the induction step for $m$.

\section*{$n=4$}
We rearrange $q$ as $q=a_1x_1^2+a_2x_2^2-a_3x_3^2-a_4x_4^2=a_1x_1^2+a_2x_2^2-(a_3x_3^2+a_4x_4^2)=q'-q''$, and we claim that for every prime $p$ there exists $a\in\mathbb{Q}_p^{\ast}$ and two vectors $u=(v_1,v_2)$ and $w=(v_3,v_4)$, such that $q'(u)=a_1v_1^2+a_2v_2^2=q''(w)=a_3v_3^2+a_4v_4^2=a$ (a full proof is given later in this document).\\
consider the quadratic form $f'=ay^2-q'=ay^2-(a_1x_1^2+a_2x_2^2)$, hence if $f'(u)=0$ then $q'(u)=f'(u)=a\implies{a}y^2-q'(u)=ay^2-a=a(y^2-1)=0$, which means that $u'=(v_1,v_2,\pm1)\in\mathbb{Q}_p^3$ is a nontrivial solution of $f'$, but this is true for all $\mathbb{Q}_p^3$, hence it is also true for $\mathbb{Q}^3$, as we already observed in the previous case $n=3$, but if $f'$ has $0$ as a value for some $u'\in\mathbb{Q}^3$ then $q'=ay^2-f'$ has the value $a$ for some $u''=(v_1,v_2)\in\mathbb{Q}^2$, and the same exactly applies for $q''$ which also has the value $a$ for some $w''\in\mathbb{Q}^2$, hence we denote $v''=u''+w''$, and then $q(v'')=q'(u'')-q''(w'')=a-a=0$, for $v''\in\mathbb{Q}^4$.
\section*{case $n>4$}
The full proof assumed that there is a nontrivial solution in $\mathbb{Q}_p$ for a finite set of primes $\{p_1,p_2,\dots,p_k\}$, but here we assume, as for the previous cases, that it is true for all primes $p$.\\
We rearrange $q$ as 
...
Suppose that $q(v)=0$ for $v=(v_1,v_2,v_3,v_4,\dots,v_n)\in\mathbb{Q}_p^n$, then there exists some $a_p\in\mathbb{Q}_p$ such that $h(v')=g(v'')=a_p$, for $v'=(v_1,v_2)$ and $v''=(v_3,v_4,\dots,v_n)$.
We claim (without proof) that the squared $p$-adic rationals $(\mathbb{Q}_p^{\ast})^2$ form an open set\footnote{because let $S=1+p\mathbb{Z}_p=\{1+py : y\in\mathbb{Z}_p\}$ for $p>2$, then for every element $a\in{S}$, we have the polynomial $f(x)=x^2-a=x^2-(1-py)=(x+1)(x-1)+py\equiv(x+1)(x-1)(\mod{p})$, also $x+1$ and $x-1$ are relatively prime modulo $p$ and monic, hence, by Hensel’s lemma for polynomials, $f(x)=(x+\sqrt{a})(x-\sqrt{a})$ where $\sqrt{a}\in\mathbb{Z}_p$, hence $a$ is a square in $\mathbb{Z}_p$, which means that $1+p\mathbb{Z}_p$ is an open neighborhood of $1$ whose elements are squares only} in $\mathbb{Q}_p^{\ast}$, and therefore, due to the approximation theorem, there exists $u=(u_1,u_2)\in\mathbb{Q}^2$ such that if $h(u)=a\in\mathbb{Q}$, then $\frac{a_p}{a}$ is a square in $\mathbb{Q}_p^{\ast}$, but then the quadratic form $g'=ax_{n+1}^2-g$ has a solution\footnote{The quadratic form $g=q-h$ is of dimension $n-2$, and we added $ax_{n+1}^2$.}  $w=(v_3,v_4,\dots,v_n,\sqrt{\frac{a_p}{a}})\in\mathbb{Q}_p^{n-1}$, because $g'(w)=a\frac{a_p}{a}-g(v'')=a_p-a_p=0$, and since $g'$ is of dimension $n-1$, then by the assumption it also has a nontrivial solution $w'=(v'_3,v'_4,\dots,v'_n,v'_{n+1})\in\mathbb{Q}^{n-1}$, but since $g=ax_{n+1}^2-g'$, the vector $w''=(v'_3,v'_4,\dots,v'_n,v'_{n+1},1)$ yields $g(w'')=a-g'(w')=a$, thus the vector $u'=(u_1,u_2,v'_1,v'_2,\dots,v'_n,v'_{n+1},1)$ yields $q(u')=h(u)-g(w'')=a-a=0$, which concludes  the proof for all $n>4$, and so we get this result for every $n>1$.
\end{proof}

\label{bijection}
\begin{proposition}
Let $a_2,a_3,y,z\in{K}$, such that $z^2-a_2=a_3y$, then
...
Let 
\begin{align*}
q'&=a_1x_1^2+a_2x_2^2+\cdots+a_rx_r^2\\ q''&=a_{r+1}x_{r+1}^2+a_{r+2}x_{r+2}^2+\cdots+a_nx_n^2
\end{align*}
be two quadratic forms such that $q=q'-q''$ has a nontrivial solution $v\in{K}^n$, then there exist $v'\in{K}^r$ and $v''\in{K}^{n-r}$ and $a\in{K}^{\ast}$ such that $q'(v')=q''(v'')=a$.
\end{proposition}
\begin{proof}
...
Consider an $n$-dimensional quadratic form $(V,q)$, where $n=m+1=2k+1$ for some $k\in\mathbb{Z}$, if there exists some $v\in{V}$ such that $q(v)=a$ for some scalar $a\in{K}^{\ast}$, then let $U=Kv<V$ be a one-dimensional subspace of $V$, and we construct an $m$-dimensional subspace $H<V$ by taking the orthogonal complement of $U$ in $V$, and we consider the quadratic space $(H,h)$, where $h=\langle1,-1,1,-1,\dots,1,-1\rangle$, for which the vector $v=(1,1,\dots,1)\in{K}^m$ is isotropic because $h(v)=1^2-1^2+\cdots+1^2-1^2=0$, hence $q\sim{h}\perp\langle{a}\rangle=\langle1,-1,\dots,1,-1,a\rangle=h+ax_{m+1}^2$ has $0$ and $a$ as values\footnote{take the vector $u=(1,1,\dots,1,1)\in{K}^{m+1}$, then $q(u)=1^2-1^2+1^2-1^2+\cdots+1^2a=a$, or take the vector $w=(1,1,\dots,1,0)\in{K}^{m+1}$, then $q(w)=1^2-1^2+1^2-1^2+\cdots+0a=0$}, but then, the quadratic form $q'=q\perp\langle-a\rangle=q-ax_{m+2}^2$ has again $0$ and $a$ as values\footnote{because if  $q(v)=a$ then taking $v_{m+1}=0$ yields $q(v)-a0=a-0=a$, and taking $v_{m+1}=1$ yields $q(v)-a1=a-a=0$, and if $q(v)=0$ then taking $v_{m+1}=0$ yields $q(v)-a0=0-0=0$.}.\\
Now suppose that there exists an arbitrary vector $u=(u_1,u_2,\dots,u_{m+2})\in{K}^{m+2}$ such that $q'(u)=0$, then either $u_{m+2}=0$ and then $q(u)=q'(u)+ax_{m+2}^2=0+a0=0$, which shows that $q$ has again $0$ and $a$ as values\footnote{We use the same equivalence $q\sim{h}\perp\langle{a}\rangle$ as before.}, or $u_{m+2}\neq0$, and then we take \[w=\frac{u}{u_{m+2}}=(\frac{u_1}{u_{m+2}},\frac{u_2}{u_{m+2}},\dots,1),\]
for which $q(w)=q'(w)+a1^2=q'(\frac{u}{u_{m+2}})+a=\frac{1}{u_{m+2}^2}q'(u)+a=0+a=a$, thus we get a circular reasoning:\\ $a$ is a value of $q\implies{q}\sim{h}\perp\langle{a}\rangle$ has $0$ and $a$ as values $\implies$ $q'=q-ax_{n}^2$ has $0$ and $a$ as values $\implies$ $a$ is a value of $q$.\\
But this means that $q$ has $0$ as a value if and only if $q$ has $a$ as a value for every $a\in{K}^{\ast}$.
Now we assume there exist two quadratic forms $q',q''$ of dimensions $r,n-r$, but for simplicity we assume that they are of the same dimension ($r=n-r=m+1$), and we denote the quadratic form of their difference by $q=q'-q''$. We saw earlier that $a$ is a value of $q'$ and of $q''$ if and only if $0$ is a value of $q'-ax_{m+1}^2$ and of $q''-ax_{m+1}^2$.
We extend $q$ to a $2(m+1)$-dimensional quadratic form $q=q'\perp-q''$, and suppose that there exist vectors $u,w\in{K}^{2(m+1)}$ such that $q'(u)=q''(w)=a\in{K}$, then we denote 
\begin{align*}
u'&=(u_1,u_2,\dots,u_{m+1},0,0,\dots,0,0)\\
w'&=(0,0,\dots,0,0,w_1,w_2,\dots,w_{m+1})\\
v&=u'+w'=(u_1,u_2,\dots,u_{m+1},w_1,w_2,\dots,w_{m+1})\\
\end{align*}
Then $q(v)=q'(u')-q''(w')=a-a=0$, and it is possible that $a=0$, but by the circular reasoning from earlier we know that $q',q''$ have $0$ as a value if and only if they have $a\neq0$ as a value, for every $a\in{K}^{\ast}$.
\end{proof}
\end{document}